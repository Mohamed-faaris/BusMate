...
import { index, pgTableCreator, primaryKey, PgArray } from "drizzle-orm/pg-core";
...
export const buses = createTable("bus", (d) => ({
  id: d
    ...
    .primaryKey()
    .$defaultFn(() => crypto.randomUUID()),
  busNumber: d.varchar({ length: 255 }).notNull().unique(),
  model: d.varchar({ length: 255 }).notNull(),
  routeName: d.varchar({ length: 255 }).notNull(),
  boardingPoints: d.PgArray(d.varchar({ length: 255 })).notNull(),
