...
export const genderFilter = (
  maxRowIncluded = 2,
  gender: "male" | "female",
  seats: Record<string, Seat["seatStatus"]>,
  modelData: BusModelProperties,
): string[] => {
  ...
  if (gender === "male") {
    ...
    let rowsIncluded = 0;
    const boysSeatGroups = [
      modelData.leftSeatColumns,
      modelData.rightSeatColumns,
    ];
    for (const seatGroup of boysSeatGroups) {
      for (
        let i = seatGroup.seatsRows.length;
        i >= 0 && rowsIncluded < maxRowIncluded;
        i--
      ) {
    ...
    for (const seatGroup of boysSeatGroups) {
      for (
        let i = 0;
        i < seatGroup.seatsRows.length && rowsIncluded < maxRowIncluded;
        i++
      ) {
        const seatRows = seatGroup.seatsRows[i]!;
        let isMaleRow = false;
